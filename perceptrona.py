...
def perceptrona(w_init, X, Y):
    #PERCEPTRONA fit sequential perceptron to data X,Y

    #PERCEPTRONA(w_init, X, Y) finds and returns the Weights that
    ...
    while (sum(errors) > 0):
        ...
        seq = seq + 1
        k = seq % m
        #cannot separate classes in allotted iteration maximum, terminate
        if (seq > maxIts):
...
def main():
    ...
    w_init = [0.0, 0.0]
    w1 = perceptrona(w_init, X1, Y1)
    print w1
    w2 = perceptrona(w_init, X2, Y2)
    print w2
    ...
    p1.scatter(X1, Y1)
    p1.set_title('Y1 v X1')
    p1.set_xlabel('x')
    p1.set_ylabel('y')
    #plot X2, Y2
    p2.scatter(X2, Y2)
    p2.set_title('Y2 v X2')
    p2.set_xlabel('x')
    p2.set_ylabel('y')
    plt.show()
